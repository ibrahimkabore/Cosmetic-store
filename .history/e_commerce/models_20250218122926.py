...
class CartItem(BaseModel):
    ...
    total_price = models.DecimalField(max_digits=10, decimal_places=0)  # adjust values as needed
    
    total_price_shoppingcart = models.PositiveIntegerField(
        _("Total Price Shopping Cart"), 
    )
    ...
    def calculate_total_price(self):
        """Calculate total price for this cart item based on product price and quantity."""
        self.total_price = self.product.price * self.quantity
        self.save()  # Don't forget to save the changes

    def calculate_total_price_shoppingcart(self):
        """Calculate total price for the entire shopping cart by summing all item prices."""
        total_cart_price = self.cart.cart_items.aggregate(
            total_price=Sum('total_price')
        )['total_price'] or 0
        self.total_price_shoppingcart = total_cart_price
        self.save()  # Don't forget to save the changes
