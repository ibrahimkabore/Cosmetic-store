...
class Order(BaseModel):
    ...
    def create_from_cart(self, shipping_address, payment_method, notes=""):
        """
        Creates an order from an active shopping cart.
        Validates cart contents and converts cart items to order items.
        """
    ...
    def validate_payment(self, transaction_id=None):
        """
        Validates order payment and updates status.
        Creates a payment record and transitions order to paid status.
        """
        if self.status not in ['pending', 'payment_waiting']:
    ...
    def process_order(self):
        """
        Starts order processing after payment confirmation.
        Updates inventory and transitions order to processing status.
        """
        if self.status != 'paid':
            raise ValidationError("Order must be paid before processing")
    ...
    def ship_order(self, tracking_number=None):
        ...
        Updates status and can handle shipping tracking information.
        """
        if self.status != 'processing':
            raise ValidationError("Order must be in processing before shipping")

        self.status = 'shipped'
        self.save()

        # Could add logic here to send customer email with tracking info

    def deliver_order(self):
        """
        Marks the order as delivered.
        Final state in the normal order workflow.
        """
    ...
    def cancel_order(self, reason=""):
        """
        Cancels the order and restores inventory if necessary.
        ...
        if previous_status in ['processing', 'shipped']:
            for item in self.items.all():
                product = item.product
                product.stock_quantity += item.quantity
                product.save()
    ...
    def can_transition_to(self, new_status):
        """
        Checks if the transition to the new status is valid.
        Implements order workflow rules.
        """
        valid_transitions = {
            'pending': ['payment_waiting', 'cancelled'],
            'payment_waiting': ['paid', 'cancelled'],
    ...
    def get_status_history(self):
        """
        Returns the order's status change history.
        Tracks all status transitions chronologically.
        """
        return self.history.filter(
            history_type='~'
        ).exclude(
            status=models.F('prev_record__status')
...
class Payment(BaseModel):
    ...
    def mark_as_refunded(self):
        """
        Marks the payment as refunded and updates the order.
        Handles the refund workflow.
        """
